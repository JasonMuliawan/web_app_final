...
def create(db: Session, user: Schemas_User):
    ...
    # if pre is None:
    #     pre = User(id=-1, plastic=user.plastic, meat=user.meat, dish=user.dish, trans=user.trans)
        # db.add(pre)
        # db.commit()
        # db.refresh(pre)
    else:
...
def get_rank(db: Session, user_id: str):
    ...
    if user.rank == 1:
        ...
        ans.plastic_num = db.query(subquery.c.rank, subquery.c.plastic).filter(subquery.c.rank==user.rank-1).first().plastic-user.plastic
        ans.plastic_rank = user.rank
    ...
    if user.rank == 1:
        ...
        ans.meat_num = db.query(subquery.c.rank, subquery.c.meat).filter(subquery.c.rank==user.rank-1).first().meat-user.meat
        ans.meat_rank = user.rank
    ...
    if user.rank == 1:
        ...
        ans.dish_num = db.query(subquery.c.rank, subquery.c.dish).filter(subquery.c.rank==user.rank-1).first().dish-user.dish
        ans.dish_rank = user.rank
    ...
    if user.rank == 1:
        ...
        ans.trans_num = db.query(subquery.c.rank, subquery.c.trans).filter(subquery.c.rank==user.rank-1).first().trans-user.trans
        ans.trans_rank = user.rank
    return ans
