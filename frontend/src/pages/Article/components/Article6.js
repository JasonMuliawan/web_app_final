    ...
    <div className="back" style={{ backgroundImage: `url(${background})` }}>
      ...
      <main>
        ...
        <section>
          ...
          環保已成為當今社會中不可或缺的議題，這也使得越來越多的人開始關注自己的<br/>
            消費行為是否對環境造成負面影響。也讓綠色消費成為了一種趨勢。<br/>
      ...
      <footer style={{ textAlign: 'center', fontSize: '20px' }}>
        <div style={{ padding: '1%' }}>
          <Link to="/article" className="last_page">回上一頁</Link>
        </div>
      </footer>
    </div>
    <div>
      ...
      <div className="border_3">
        <div id="comment-section6" style={{ maxHeight: '150px', overflowY: 'scroll' }}>
          {comments.map((comment, index) => (
            <div className="comment-box" key={index}>
              <span style={{ color: '#0d9156', fontSize: '20px', fontWeight: 800 }}>暱稱：</span>
              <span style={{ fontWeight: 700, fontSize: '20px', color: '#2f45d6' }}>{comment.name}</span>
              &emsp;&emsp;
        ...
        <label htmlFor="name" style={{fontSize: '16px', fontWeight: 600}}>名稱：</label><br />
          ...
          <button type="button" onClick={() => {addComment6();}} style={{fontSize: '16px', backgroundColor: '#97bcf7', fontWeight: 600}}>發佈評論</button>
          </div>
        </form>
      </div>
    </div>
